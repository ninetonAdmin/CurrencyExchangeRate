...
{
    DLog(@"WARNING: using default weak obfuscation. Provide your own obfuscator if piracy is a concern.");
           ...
           failure:(RMSKPaymentTransactionFailureBlock)failureBlock
{
    ...
    if (product == nil)
    {
        DLog(@"unknown product id %@", productIdentifier);
        if (failureBlock != nil)
        {
...
{
    DLog(@"restore transactions finished");
...
{
    DLog(@"restored transactions failed with error %@", error.debugDescription);
...
{
    DLog(@"transaction purchased with product %@", transaction.payment.productIdentifier);
    ...
    {
        DLog(@"WARNING: no receipt verification");
        [self paymentQueue:queue verifiedTransaction:transaction];
    }
}
...
- (void)paymentQueue:(SKPaymentQueue *)queue failedTransaction:(SKPaymentTransaction *)transaction error:(NSError*)error
{
    SKPayment *payment = transaction.payment;
	NSString* productIdentifier = payment.productIdentifier;
    DLog(@"transaction failed with product %@ and error %@", productIdentifier, error.debugDescription);
...
{
    DLog(@"transaction restored with product %@", transaction.originalTransaction.payment.productIdentifier);
...
{
//    DLog(@"products request received response");
    ...
    {
//        DLog(@"received product with id %@", product.productIdentifier);
        [self.store addProduct:product];
    }
    
//    for (NSString *invalid in invalidProductIdentifiers)
//    {
////        DLog(@"invalid product with id %@", invalid);
//    }
...
{
    DLog(@"products request failed with error %@", error.debugDescription);
