...
{    
    ...
    if (error != nil)
    {
        DLog(@"Failed to serialize receipt into JSON");
        if (failureBlock != nil)
        {
            failureBlock(error);
        }
                  ...
                  failure:(void (^)(NSError *error))failureBlock
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (jsonError != nil)
            {
                DLog(@"Failed To Parse Server Response");
                if (failureBlock != nil)
                {
                    failureBlock(error);
                }
            ...
            {
                DLog(@"Verifying Sandbox Receipt");
            ...
            {
               // DLog(@"Verification Failed With Cod%ld%d"(long), statusCode);
